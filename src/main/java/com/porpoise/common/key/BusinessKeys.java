...
public class BusinessKeys<T>
{
	...
	private BusinessKeys(final Class<T> c1ass)
	{
		...
		ConcurrentMap<String, Map<String, BusinessKeyAccessor<T, Object>>> byType = new MapMaker().makeComputingMap(new Function<String, Map<String,BusinessKeyAccessor<T,Object>>>() {
			@Override
			public Map<String, BusinessKeyAccessor<T, Object>> apply(
					String input) {
				return computeKeyByName(input);
			}
		});
		Map<String, BusinessKeyAccessor<T, Object>> defaultMap = Collections.emptyMap();
		keysByType = Functions.forMap(byType, defaultMap);
	}
	...
	{
		boolean hasType = !Strings.isNullOrEmpty(type);
		...
		for (final BusinessKeyAccessor<T, Object> accessor : businessKeysForType(type))
		{
			...
			String str;
			Object obj = accessor.apply(instance);
			if (hasType && obj instanceof BusinessEquality)
			{
				BusinessEquality be = (BusinessEquality) obj;
	...
	{
		Map<String, BusinessKeyAccessor<T, Object>> map = lookupForType(type);
		if (map.isEmpty())
		{
		...
		Function<Pair<Integer, Object>, Integer> hash = new Function<Pair<Integer,Object>, Integer>() {
			@Override
			public Integer apply(Pair<Integer, Object> input) {
				int h = input.getFirst().intValue();
				Object obj = input.getSecond();
				int newHash;
				...
				return Integer.valueOf(h + newHash);
			}
		};
		Integer hashCode = Sequences.foldLeft(Integer.valueOf(31), values, hash);
		return hashCode.intValue();
	}

	private Map<String, BusinessKeyAccessor<T, Object>> lookupForType(
			String type) {
		Map<String, BusinessKeyAccessor<T, Object>> map;
		if (Strings.isNullOrEmpty(type))
		map = keysByType.apply(type);
		else {
			map = keyByName;
		}
		return map;
	...
	private static  boolean typeAwareEquals(final String type, final Object o1,
			final Object o2) {
		...
		else if (o1 instanceof Collection<?>)
		{
			...
			Collection<Object> collectionOne = (Collection<Object>) o1;
			Collection<Object> collectionTwo = (Collection<Object>) o2;
	 ...
	 * @return
	 */
	private Map<String, Pair<Object, Object>> diff(String type, final Map<String, BusinessKeyAccessor<T, Object>> keys,
			final T first,
			final T second)
	{
	...
	static Predicate<Pair<Object, Object>> different(final  String type) {
		return new Predicate<Pair<Object,Object>>() {
			@Override
			public boolean apply(Pair<Object, Object> input) {
	...
	{
		final Map<String, BusinessKeyAccessor<T, Object>> filteredDeyByName = this.keysByType.apply(type);
	...
	public Map<Object, T> groupUnique(final Iterable<T> objects)
	{
		...
		catch (final IllegalArgumentException notUnique)
		{
			...
			throw new IllegalArgumentException(String.format("%s%nBusiness Values Are:%n%s",
					notUnique.getMessage(), error));
		}
	}
	...
	class HashKey
	{
		...
		@SuppressWarnings("synthetic-access")
		HashKey(String type, final T obj)
		{
			this.type = type;
		...
		public boolean equals(final Object other)
		{
			...
			if (other instanceof BusinessKeys.HashKey)
			{
				...
				{
					return BusinessKeys.this.equals(type, this.wrappedObject, (T) otherKey.wrappedObject);
				}
			}
			return false;
		}
		...
		public String toString()
		{
			return "KEY FOR " + BusinessKeys.this.toString(type, this.wrappedObject);
		}
	}
	 ...
	 */
	public Object makeKey(String type, final T obj)
	{
	 ...
	 */
	public <K> Map<K, T> groupByUnique(final String propertyName, final Iterable<T> values)
	{
		final Function<T, K> fnc = groupingFunctionForProperty(propertyName);
	 ...
	 * @return the values by the given property
	 */
	public <K> Map<K, Collection<T>> groupBy(final String propertyName, final Iterable<T> values)
	{
		final Function<T, K> fnc = groupingFunctionForProperty(propertyName);
